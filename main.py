...
app = FastAPI(
    title="PDF Extractor (Gemini-only)",
    version="1.1.2",
)
...
async def extract_endpoint(
    ...
    try:
        from extractor import (
            extract_pdf_content,
            build_single_text,
            gemini_extract_fields_only,    # Data 1 (Wage Tracker)
            gemini_extract_factory_review, # Data 2 (Factory Review)
            extract_audit_date,            # Regex-based audit date helper
        )
    ...
    try:
        ...
        # ---------- Data 1: Wage Tracker ----------
        data1 = gemini_extract_fields_only(doc_text)

        # ---------- Data 2: Factory Review ----------
        data2 = gemini_extract_factory_review(doc_text)

        # Try to override audit_date from regex if we find a clearer value
        ...
        if include_text_preview:
            combined["preview"] = doc_text[:1000] + ("…[truncated]" if len(doc_text) > 1000 else "")
            combined["meta"] = {
                "pages": len(pages),
